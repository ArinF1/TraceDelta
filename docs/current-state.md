...
Validated on 2026-07-21 after TD-031. The baseline example generated byte-deterministic valid JSON and self-compared with exit `0`; the candidate generated exactly one added span, one removed span, one status change, and one latency finding with exit `1`. Local Action execution preserved non-empty text, JSON, and HTML reports. Public draft PR #7 passed its normal Linux and Windows jobs, failed only its intentional `compare-example` gate, and published artifact `example-tracedelta-reports` containing all three non-empty formats. The repository-native Windows entry point passed formatting, tests across all ten packages, vet, and CLI build; the race-enabled suite passed all ten packages before foundation publication.
...
**TD-032 — Complete the v0.1 verification matrix.** Add bounded parser/matcher/reporter fuzz smoke and finish end-to-end CLI/Action result-consistency gates without duplicating existing unit coverage.
