...
- A synthetic example application, deliberately regressed pull request, versioned release binaries, or 45–60 second demonstration.
...
Validated on 2026-07-21 after TD-015. The current-source binary built successfully. The generic CI smoke passed no-difference (`0`), behavioral-difference (`1`), invalid-input (`2`), and missing-input (`2`) paths with complete reports only for successful comparisons. The Action entry-point smoke passed regression report/output handling and missing-input failure/output handling. Bash syntax and YAML parsing passed for the Action, scripts, repository CI, and sample workflow. The repository-native Windows entry point passed formatting, tests across all nine packages, `go vet ./...`, and CLI build; `go test -race -count=1 ./...` passed all nine packages.
...
**TD-031 — Add the deliberately regressed example pull request.** Add a deterministic synthetic example application and replayable regression workflow that exercises every v0.1 finding category through the reusable Action.
